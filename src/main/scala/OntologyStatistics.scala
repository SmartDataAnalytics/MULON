...
class OntologyStatistics(sparkSession: SparkSession) {
  def GetStatistics(ontologyTriples: RDD[graph.Triple]) = {
    ...
    println("Number of triples in the " + ontoName + " ontology = " + ontologyTriples.count()) //    triples.foreach(println(_))
  ...
  def RetrieveClassesWithCodesAndLabels(ontologyTriples: RDD[graph.Triple]): RDD[(String, String)] = {
    ...
    var classes = ontologyTriples.find(None, None, Some(NodeFactory.createURI("http://www.w3.org/2002/07/owl#Class"))).keyBy(_.getSubject.getLocalName).join(ontologyTriples.keyBy(_.getSubject.getLocalName)).filter(x => x._2._2.getPredicate.getLocalName == "label").map(y => (y._1, y._2._2.getObject.getLiteral.getLexicalForm.split("@").head)).distinct(2)
    classes
  }
