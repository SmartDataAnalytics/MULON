...
class TestClassSimilarity {
  ...
  def GetSimilarity(O1Translated: RDD[(String, String)], O2: RDD[String])={
    val gS = new GetSimilarity()
    val t = O1Translated.cartesian(O2)
    val tt = t.map(x => (x._1._1, x._1._2, x._2,gS.getSimilarity(x._1._2.toLowerCase,x._2.toLowerCase))).filter(y=>y._4>0.9)
