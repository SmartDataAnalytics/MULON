...
class OntologyMerging(sparkSession: SparkSession) {
  ...
  def Merge(sourceClassesWithBestTranslation: RDD[(String, String, String)], targetClassesWithoutCodes: RDD[String], listOfMatchedClasses: RDD[List[String]],similarRelations: RDD[(String, String, String, String, Double)], relationsWithTranslation: RDD[(String, String, String)],sOntology: RDD[(String, String, String)]) = {
    ...
    val RelationsWithMultilingualInfo = this.ResolveConflictRelations(similarRelations, relationsWithTranslation)
    ...
    val sourceRelationsWithURIs = this.GenerateURIsForResources(RelationsWithMultilingualInfo.map(x => (x._2, x._3)), 'P')
    //    println("Source relations with URIs"+sourceRelationsWithURIs.count())
    //    sourceRelationsWithURIs.foreach(println(_))

    val translatedOntology: RDD[graph.Triple] = this.TranslateSourceOntology(sourceClassesWithURIs.union(sourceRelationsWithURIs), sOntology)
    println("Translate ontology with number of triples")
    translatedOntology.foreach(println(_))
//    translatedOntology.coalesce(1, shuffle = true).saveAsNTriplesFile("src/main/resources/Output/MultilingualSourceOntology")


    val englishLabels = gCreate.CreateMultilingualEnglishLabels(sourceClassesWithURIs.union(sourceRelationsWithURIs))
    //    println("English labels"+englishLabels.count())
    //    englishLabels.foreach(println(_))

    val germanLabels = gCreate.CreateMultilingualGermanLabels(sourceClassesWithURIs.union(sourceRelationsWithURIs))
//    println("German labels"+germanLabels.count())
//    germanLabels.foreach(println(_))

    val allSourceLabelsGraph = englishLabels.union(germanLabels)
  }
  ...
  def GenerateURIsForResources(allResourcesWithMultilingualInfo: RDD[(String, String)], resourceType: Char): RDD[(String, String, String)] = {
    ...
    if (resourceType == 'C')
      resourcesWithURIs = allResourcesWithMultilingualInfo.map(x => ("https://merged-ontology#"++p.ToCamelForClass(x._2), x._1, x._2))
    else if (resourceType == 'P')
      resourcesWithURIs = allResourcesWithMultilingualInfo.map(x => ("https://merged-ontology#"++p.ToCamelForRelation(x._2), x._1, x._2))
    resourcesWithURIs
  }
