...
* */ class RelationSimilarity {
  ...
  def GetRelationSimilarity(listOfRelationsInTargetOntology: RDD[(String)], listOfRelationsInSourceOntology: RDD[(String, String, String)]) = {
    ...
    var sim: RDD[(String, String, String, String, Double)] = crossRelations.map(x => (x._2._1, x._2._2, x._2._3, x._1, gS.getSimilarity(p.removeStopWordsFromEnglish(p.splitCamelCase(x._1).toLowerCase), p.removeStopWordsFromEnglish(p.splitCamelCase(x._2._3).toLowerCase)))).filter(y => y._5 > 0.7)
    sim
  }
