...
object Try {
  def main(args: Array[String]): Unit = {
    ...
    val ontoRebuild = new OntologyRebuilding(sparkSession)
    ...
    val tOntology = ontoRebuild.RebuildOntology(O2triples) //    println("O2:")
    ...
    val O1Relations: RDD[String] = ontoStat.getAllRelations(O1LabelsBroadcasting, O1triples).map(x => x._2)
    ...
    val O2LabelsBroadcasting: Broadcast[Map[Node, graph.Triple]] = sparkSession.sparkContext.broadcast(O2Labels)

    val O2Relations: RDD[String] = ontoStat.getAllRelations(O2LabelsBroadcasting, O2triples).map(x => x._1)
